  ...
  "questions": [
    {
      ...
      "choices": {
        "A": "沒事 —— CPU 使用率是 inference 推論的標準訊號",
        "B": "Autoscaler 會 overscale,因為 CPU 在跑背景",
        "C": "GPU 是瓶頸,用 tokens/sec、queue depth、KV cache %",
        "D": "只有 nvidia-smi % 可靠,其他訊號無法信任"
      },
    ...
    },
    {
      ...
      "choices": {
        "A": "Kubernetes control plane 的 scaling API 本身就不夠快速",
        "B": "LLM pod cold-start 要數分鐘:image pull、權重載、CUDA graph",
        "C": "Kubernetes autoscaler 從根本上無法排程 GPU 型 workload",
        "D": "新的 replica 必須重啟硬體機器才能投入服務使用"
      },
    ...
    },
    {
      ...
      "choices": {
        "A": "一張大 GPU 上跑多個小模型,租戶間有硬體隔離",
        "B": "伺服一個大模型,要用整張 GPU 的 HBM",
        "C": "跨多 node 的大規模訓練",
        "D": "Apple Silicon edge 部署"
      },
    ...
    },
    {
      ...
      "choices": {
        "A": "TP 需要獨佔的專屬 node,無法與其他工作共存",
        "B": "Tensor-parallel 只能在 AMD GPU 上執行",
        "C": "TP 需要同一 node 的多張 GPU,用 nodeSelector 限制",
        "D": "Tensor-parallel 必須獨佔 preemptible instance"
      },
    ...
    },
    {
      ...
      "choices": {
        "A": "降低新版本的總記憶體占用和 overhead",
        "B": "新版有品質問題時能瞬間 rollback,冷啟動要數分鐘",
        "C": "Kubernetes cluster 規定必須同時執行兩個版本",
        "D": "舊版本在不同的硬體一代上執行"
      },
