  ...
  "questions": [
    {
      ...
      "choices": {
        "A": "現代模型訓練時就預期這種 loss,所以權重結構有充分冗餘",
        "B": "Benchmark 本身對精度不敏感,大多偏 robust",
        "C": "LLM 權重高度集中在 0 附近、偶有離群值 —— 把 bit 放在質量聚集處能用很少 bit 抓到重點",
        "D": "4-bit 儲存其實會在量化之上又疊加 lossless 壓縮"
      },
    ...
    },
    {
      ...
      "choices": {
        "A": "兩者一模一樣 —— exponent 和 mantissa 配置完全相同",
        "B": "BF16 有 FP32 的寬動態範圍但精度低;FP16 精度好但範圍窄、訓練易 overflow",
        "C": "BF16 每個值 4 bytes,FP16 是 2 bytes,記憶體佔用差兩倍",
        "D": "BF16 只在 AMD 上跑,FP16 只在 NVIDIA tensor core 支援"
      },
    ...
    },
    {
      ...
      "choices": {
        "A": "INT4/INT8 存的權重即時 dequant 回 BF16/FP16,再和 BF16 activation 相乘",
        "B": "所有東西(包含 activation)都保留在 INT4,做整數乘法",
        "C": "GPU 的 tensor core 直接做 INT4 × INT4 的乘法,完全不 dequant",
        "D": "權重保持 INT4,activation 也要先量化成相同精度才能算"
      },
    ...
    },
    {
      ...
      "choices": {
        "A": "Round 完之後再對權重套上一層 lossless 壓縮編碼",
        "B": "整個 model 在低精度下重新從頭訓練一次,以對齊量化網格",
        "C": "重新設計傳輸權重用的網路協定,讓量化效率更高",
        "D": "拿一小批代表性樣本跑過 model,量測實際分布,再挑保留重要質量的 per-layer scale"
      },
    ...
    },
    {
      ...
      "choices": {
        "A": "~70 GB,大致是 BF16 大小的一半左右",
        "B": "~18 GB,因為 INT4 還會多一層壓縮",
        "C": "~35 GB,BF16 的四分之一,塞得進一張 H100",
        "D": "差不多 140 GB —— 量化不會減少權重儲存空間"
      },
    ...
    },
    {
      ...
      "choices": {
        "A": "大規模 NVIDIA 資料中心搭配 vLLM,要每張卡的最高 throughput",
        "B": "Apple Silicon、純 CPU、edge/桌面,用 llama.cpp 跑、統一記憶體",
        "C": "H100 上的訓練和 fine-tune,要 BF16 的數值範圍",
        "D": "QLoRA 的 fine-tune 訓練,需要動態量化和 dequant"
      },
